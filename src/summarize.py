...
3. **마지막 항목은 반드시 `시사점: ` 으로 시작한다.** 기사에 적힌 사실에서
...
4. **기사가 부실하면 그렇다고 적어라.** 메커니즘을 알 수 없으면 시사점 항목에
   "기사에 근거가 부족함" 이라고 쓴다. 억지로 만들어내지 마라.

5. **큰 숫자가 나오면 이번 발표분을 갈라 적어라.** 보도자료는 누적 실적과 신규
   계약을 한 문장에 섞어 쓴다. "기존 협력 포함 1,200MW, 이번 신규 추가는 144MW"
   처럼 구분하지 않으면 독자가 규모를 열 배로 오해한다. 계약의 성격도 마찬가지다
   — 환경가치(REC) 구매인지, 24시간 전력공급 계약인지 구분해서 적어라.

6. **무엇이 공개되지 않았는지 한 줄로 밝혀라.** 계약기간·단가·접속 조건·지분율처럼
...
emoji   : 기사 주제에 맞는 것 하나.
          ⚛️ 원전·SMR   ⚡ 전력·유틸리티·요금   🔌 전력기기·송배전·케이블
          🏗️ 건설·플랜트·EPC   ☀️ 재생에너지   🔋 배터리·ESS·수소
...
SCHEMA = {
    ...
    "required": ["publish", "importance", "reject_reason", "title", "emoji",
                 "lead", "details", "tags"],
